  ...
  return (
    <ol className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
      {steps.map((s, i) => (
        ...
        >
          <div className="mb-4 grid h-9 w-9 place-items-center rounded-lg bg-primary text-sm font-semibold text-primary-foreground shadow-soft">
            {i + 1}
          </div>
    ...
    <ol className="relative border-l border-border pl-8">
      {items.map((it, i) => (
        <li key={i} className="relative pb-10 last:pb-0">
          <span className="absolute -left-[37px] top-1 grid h-6 w-6 place-items-center rounded-full border border-border bg-white text-[11px] font-semibold text-primary shadow-soft">
            {i + 1}
          </span>
          {it.tag && (
            <p className="text-xs font-semibold uppercase tracking-widest text-primary">{it.tag}</p>
          )}
      ...
      {items.map((c) => {
        const inner = (
          <div className="group h-full rounded-2xl border border-border bg-card p-6 shadow-soft transition-all hover:-translate-y-1 hover:shadow-elevated">
            {c.tag && (
              <span className="inline-flex rounded-full border border-border bg-white px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
                {c.tag}
              </span>
            )}
            {c.icon && (
        ...
        <div className="col-span-4 space-y-3">
          ...
          <div className="space-y-1.5">
            {[
              ...
              >
                <span>{r}</span>
                <span className="rounded-full bg-white px-2 py-0.5 text-[10px] text-muted-foreground border border-border">
                  just now
                </span>
              </div>
            ))}
