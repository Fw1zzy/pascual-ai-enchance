  ...
  head: () => ({
    meta: [
      { title: "Pricing — Pascual.ai" },
      {
        name: "description",
        content:
          "Flexible plans for teams of every size — from growing companies to global enterprises.",
      },
    ],
  }),
...
const comparison = [
  {
    ...
    rows: [
      ...
      ["Agents & workflows", "Core", "Advanced", "Advanced + custom"],
      ["Integrations", "Standard", "All", "All + custom"],
    ],
  },
  {
  ...
  },
  {
    section: "Support",
    rows: [
      ["Support channel", "Email", "Priority", "24/7 + CSM"],
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        primary={{ label: "Contact sales", href: "/resources/contact" }}
        secondary={{ label: "See enterprise", href: "/products/enterprise-ai-solutions" }}
      />
      <Section>
        <div className="grid gap-6 lg:grid-cols-3">
          {tiers.map((t) => (
            ...
            >
              {t.featured && (
                <span className="absolute -top-3 left-8 rounded-full bg-primary px-2.5 py-1 text-[10px] font-semibold uppercase tracking-widest text-primary-foreground">
                  Most popular
                </span>
              )}
        ...
        <div className="mx-auto mt-10 max-w-5xl overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
          <table className="w-full text-sm">
            ...
            <tbody>
              {comparison.map((s) => (
                <>
                  <tr key={s.section} className="border-t border-border bg-muted/30">
                    <td
                      colSpan={4}
                  ...
                  {s.rows.map((r, i) => (
                    <tr key={s.section + i} className="border-t border-border">
                      <td className="px-6 py-3 font-medium">{r[0]}</td>
                      <td className="px-6 py-3 text-muted-foreground">{r[1]}</td>
                      <td className="px-6 py-3 text-muted-foreground">{r[2]}</td>
                      <td className="px-6 py-3 text-muted-foreground">{r[3]}</td>
                    </tr>
                  ))}
                </>
              ))}
